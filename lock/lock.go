...
var (
	...
	ErrFailedToPreemptLock = errors.New("lock: 抢锁失败")
	// ErrLockNotHold 一般是出现在你预期你本来持有锁，结果却没有持有锁的地方
	// 比如说当你尝试释放锁的时候，可能得到这个错误
	// 这一般意味着有人绕开了 lock 的控制，直接操作了 Redis
	ErrLockNotHold = errors.New("lock: 未持有锁")
)
...
func (c *Client) SingleflightLock(ctx context.Context, key string, expiration time.Duration, retry restry.RetryStrategy, timeout time.Duration) (*Lock, error) {
	for {
		flag := false
...
// - 否则，和 Redis 通信出了问题
func (c *Client) Lock(ctx context.Context, key string, expiration time.Duration, retry restry.RetryStrategy, timeout time.Duration) (*Lock, error) {
	val := c.valuer()
